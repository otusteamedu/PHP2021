...
Nested Loop  (cost=0.70..23.75 rows=211 width=41)
  ->  Index Scan using sessions_hall_id_date_time_key on sessions se  (cost=0.28..8.30 rows=1 width=24)
        Index Cond: ((hall_id = 1) AND (date = '2010-01-01'::date) AND ("time" = '09:00:00'::time without time zone))
  ->  Index Scan using sales_index_session_id on sales sa  (cost=0.42..13.25 rows=219 width=17)
...
Nested Loop  (cost=0.70..23.75 rows=211 width=41)
  ->  Index Scan using sessions_index_date on sessions se  (cost=0.28..8.30 rows=1 width=24)
        Index Cond: (date = '2010-01-01'::date)
        Filter: (("time" = '09:00:00'::time without time zone) AND (hall_id = 1))
  ->  Index Scan using sales_index_session_id on sales sa  (cost=0.42..13.25 rows=219 width=17)
        Index Cond: (session_id = se.id)
