...
```shell
ssh-keygen
# >> /home/$USERNAME/.ssh/id_rsa_autodeploy for example /home/viktar/.ssh/id_rsa_autodeploy

ssh-copy-id -i ~/.ssh/id_rsa_autodeploy.pub root@46.101.131.175
# or
# cat ~/.ssh/id_rsa.pub | ssh username@remote_host "mkdir -p ~/.ssh && touch ~/.ssh/authorized_keys && chmod -R go= ~/.ssh && cat >> ~/.ssh/authorized_keys"
## my case: cat ~/.ssh/id_rsa_autodeploy.pub | ssh root@46.101.131.175 "mkdir -p ~/.ssh && touch ~/.ssh/authorized_keys && chmod -R go= ~/.ssh && cat >> ~/.ssh/authorized_keys"
## my case: cat ~/.ssh/id_rsa_autodeploy.pub | ssh root@46.101.131.175 "mkdir -p /home/autodeploy/.ssh && touch /home/autodeploy/.ssh/authorized_keys && chmod -R go= /home/autodeploy/.ssh && cat >> /home/autodeploy/.ssh/authorized_keys"
### on server (cloud): echo %public_key_string% >> ~/.ssh/authorized_keys
### chmod -R go= ~/.ssh
#### sudo chown -R $USERNAME:$USERNAME ~/.ssh
#### в реале пришлось писать: sudo chown -R autodeploy:autodeploy ~/.ssh
##### и вот только теперь заработало: ssh autodeploy@46.101.131.175

# in cloud:
ssh-keygen
# >> /home/autodeploy/.ssh/id_rsa_githubaction
cat /home/autodeploy/.ssh/id_rsa_githubaction >> .ssh/authorized_keys



# test: ssh -i ~/.ssh/id_rsa_autodeploy.pub root@46.101.131.175
