...
CREATE INDEX cinema_sessions_movie_screen_idx ON cinema_sessions USING btree (movie_id, screen_id);
CREATE INDEX cinema_tickets_status_idx ON cinema_tickets USING btree (status) WHERE status = 0;
CREATE INDEX cinema_tickets_session_idx ON cinema_tickets USING btree (session_id);
