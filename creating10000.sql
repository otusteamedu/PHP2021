...
begin
	...
	select
	  CONCAT(floor(random()*(max_id_halls-min_id_halls+1)+min_id_halls))::int4,
	...
	select
	  CONCAT(to_timestamp(1633246568 + (gs.id * 10000)))::timestamp,
	  CONCAT(to_timestamp(1633246568 + (gs.id * 10000) + 3600 + (random() * (10800 - 5400))))::timestamp,
	  CONCAT(floor(random()*(max_id_halls-min_id_halls+1)+min_id_halls))::int4,
	  CONCAT(floor(random()*(max_id_movies-min_id_movies+1)+min_id_movies))::int4,
	...
	select
		CONCAT(floor(random()*(max_id_seats-min_id_seats+1)+min_id_seats))::int4, 
		CONCAT(floor(random()*(max_id_sessions-min_id_sessions+1)+min_id_sessions))::int4,
		CONCAT(floor(random()*(max_id_clients-min_id_clients+1)+min_id_clients))::int4,
		CONCAT(floor(random()*(500-150+1)+150))::int4
