...
CREATE OR REPLACE VIEW public.film_attributes_values
AS SELECT f.name,
   ...
   FROM attribute_type attr_t
     JOIN attribute attr ON attr_t.attribute_type_id = attr.attribute_type_id
     JOIN film_attributes f_attr ON attr.attribute_id = f_attr.attribute_id
     JOIN film f ON f_attr.film_id = f.film_id
